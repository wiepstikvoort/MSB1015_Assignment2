...
SERVICE wikibase:label { bd:serviceParam wikibase:language "[AUTO_LANGUAGE],en". } }')
```
The amino acids and their masses were loaded from WikiData using SPARQL. All compound masses were expressed in the atomic mass unit (which is equivalent to g/mol), so no need for conversions. 
``` {r chunk, echo = FALSE}
