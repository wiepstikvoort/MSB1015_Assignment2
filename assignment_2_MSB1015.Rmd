...
descriptors 
```{r}
parsed_smiles <- parse.smiles(query_new2$CC, kekulise=TRUE) # changes it into a format IAtomContainer
...
``` {r}
model <- plsr(query_new2$bp ~ descs$WPATH)
# voorspellen op basis van je testdata
predict <- predict(model, descs)
plot(predict)
```
